...
| `'$PORT' is not a valid integer` at container start | The start command reached Streamlit without being shell-expanded. Railway execs `railway.json`'s `startCommand` and the `Procfile` directly, not through a shell, so both wrap themselves in `sh -c '…'`. Keep that wrapper; the braced `${PORT:-8501}` alone is not enough |
| Fixes have no effect and the build log says `fetching snapshot` with every layer `cached` | The service is deploying an **uploaded snapshot** (`railway up`) rather than the GitHub repo, so pushed commits are invisible to it. Compare `containerimage.digest` across builds — if it is unchanged, the same source was rebuilt. Connect the service to GitHub (Settings → Source → Connect Repo, branch `main`), or redeploy with `railway up` from an up-to-date working copy |
